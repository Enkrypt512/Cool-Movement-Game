...
func _process(delta: float) -> void:
	...
	Current_Rotation = Current_Rotation.lerp(Target_Rotation, 1.0 - exp(-Snap_Amount * delta))
...
func Add_Recoil(Recoil_Values: Vector3, Custom_Snap_Amount: float = 15.0, Custom_Return_Speed: float = 3.0) -> void:
	...
	Target_Rotation = (Target_Rotation + Impulse).clamp(-Max_Recoil_Angles, Max_Recoil_Angles)
